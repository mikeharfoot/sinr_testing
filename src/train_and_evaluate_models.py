...
train_params[
    "experiment_name"
] = "rarefy_snt"  # This will be the name of the directory where results for this run are saved.
...
for eval_type in ["snt", "geo_prior", "geo_feature"]:
    ...
    np.save(
        os.path.join(
            eval_params["exp_base"], train_params["experiment_name"], f"results_{eval_type}.npy"
        ),
        cur_results,
    )
