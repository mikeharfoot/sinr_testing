...
def main(eval_params):
    ...
    with torch.no_grad():
        preds = (
            model(locs_enc, return_feats=False, class_of_interest=class_of_interest).cpu().numpy()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="./pretrained_models/model_an_full_input_enc_sin_cos_hard_cap_num_per_class_1000.pt",
    )
    parser.add_argument("--taxa_id", type=int, default=130714, help="iNaturalist taxon ID.")
    parser.add_argument(
        "--threshold", type=float, default=-1, help="Threshold the range map [0, 1]."
    )
    parser.add_argument(
        "--op_path",
        type=str,
    ...
    parser.add_argument(
        "--high_res", action="store_true", help="Generate higher resolution output."
    )
    parser.add_argument(
        "--disable_ocean_mask", action="store_true", help="Do not use an ocean mask."
    )
    parser.add_argument(
        "--set_max_cmap_to_1", action="store_true", help="Consistent maximum intensity ouput."
    )
